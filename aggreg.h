...
const int16s MIN_PACKET = 64;
const int16s MAX_PACKET = 1518;
...
public:
	...
	bytestamp_t byte_stamp;

    Trace()                                 { TimeStamp = 0.0; PacketSize = 0; byte_stamp = 0;}
    Trace(timestamp_t ts, pct_size_t ps, bytestamp_t bs)   
	{ 
		TimeStamp = ts; 
		PacketSize = ps; 
		byte_stamp = bs;
...
{
    return ( os << trc.TimeStamp << "    " << trc.PacketSize<<"    "<<trc.byte_stamp );
}
...
public:
    ...
    // NOTES:
    ////////////////////////////////////////////////////////////////////////////////
    virtual ~Generator()        { RemoveSources(); }
    ...
    Trace GenerateTrace( void )
    {
        ...
        Trace Trc;//(0.0, pNextSrc->GetPctSize(), round(Elapsed));
		...
		Trc.PacketSize = pNextSrc->GetPctSize();
